...
from dtw_silpau import mydtw_endpointfree, mydtw
...
def loadsilpau(fn,fshift_ms):
    with open(fn) as fp:
        ls = fp.read().rstrip('\n').split('\n')

    ls = [(float(x.split()[0]),float(x.split()[1])) for x in ls]
    ...
    for l in ls:
        lframes = list(
            range(
                int(l[0]*1000/fshift_ms),
                int(l[1]*1000/fshift_ms)-2
                )
            )

        frames.extend(lframes)
...
def main():
    ...
    parser.add_argument(
       '--silpau-path','-pau', type=str,required=True,help='path to sil/pau time files'
        )
    ...
    if args.debug:
        wavFileNames[0] = wavFileNames[0][:10]
        wavFileNames[1] = wavFileNames[1][:10]
    ...
    #load sil/paus
    # silpaul = ['{}/{}.txt'.format(args.silpau_path,i+1) for i in range(wavFileNum)]
    # silpaul = sorted(glob.glob('{}/*.txt'.format(args.silpau_path)))
    silpaul = ['{}/{}.txt'.format(args.silpau_path,os.path.basename(x).split('.')[0]) for x in wavFileNames[0]]

    for silpaufn in silpaul:
        assert os.path.exists(silpaufn),silpaufn
    ...
    for f in range(wavFileNum):
        ...
        silpaufn = silpaul[f]
        silpauframes = loadsilpau(silpaufn,fshift_ms)

        assert max(silpauframes) > len(f02),'for {}, mht signal is shorter than the label.'.format(fname_head2)
        ...
        if args.use_dim0:
        	...
        	path_r, path_c, tmpdis,D = mydtw(wmc1_conv[:,1:], wmc2[:,1:], silpauframes,dist=mydis, w=np.inf, p=penalty)
